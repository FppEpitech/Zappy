...
class Player:
    ...
    def connectMissingPlayers(self, host, port, teamName):
        """
        Connect the missing players
        """
        print("Connecting missing players", flush=True, file=sys.stderr)
        for _ in range(0, min(self.unusedSlots, 5)):
            from AI import forkAI
            forkAI(host, port, teamName)


    def completeTeam(self, host, port, teamName):
        """
        Complete the team
        """
        self.connectNbr(self.connectMissingPlayers(host, port, teamName))
    ...
    def chooseAction(self, teamName : str, myuuid : str, creationTime : int, host : str, port : int):
        """
        Choose the action of the player
        ...
        if self.currentMode == Mode.FOOD:
            ...
            print("Forking")
            from AI import forkAI
            self.fork(forkAI(host, port, teamName))
            self.nbSlaves += 1
            self.cmdInventory()
            return
