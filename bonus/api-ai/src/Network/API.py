...
from Utils.Utils import stringifyData
...
class API:
    ...
    def receiveData(self, timeout : float = None):
        ...
        for s in readable:
            if s == self.sock:
                ...
                if not data:
                    print("Server disconnected", flush=True, file=sys.stderr)
                    sys.exit(0)
                else:
    ...
    def initConnection(self, teamName : str, fileName : str = ""):
        ...
        print("Connected to server", flush=True, file=sys.stderr)
        print(f"Client number: {clientNum}", flush=True, file=sys.stderr)
        print(f"Map size: x = {x}, y = {y}", flush=True, file=sys.stderr)
