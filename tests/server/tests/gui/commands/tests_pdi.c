...
Test(pdi_command, basic_test)
{
    ...
    pdi_command(&app, 1);
    cr_assert_str_eq(gui.list_messages->first->data.message, "pdi 1\n");
}

Test(pdi_command, basic_test_2)
{
    app_t app;
    gui_t gui;
    node_data_t gui_data;
    ...
    gui_data.gui = &gui;
    list_add_back(app.gui_list, gui_data);
    pdi_command(&app, 58975);
    cr_assert_str_eq(gui.list_messages->first->data.message, "pdi 58975\n");
}

Test(pdi_command, multiple_gui_test)
{
    app_t app;
    gui_t gui;
    node_data_t gui_data;
    ...
    while (node) {
        cr_assert_str_eq(node->data.message, "pdi 1\n");
        node = node->next;
    }
}
