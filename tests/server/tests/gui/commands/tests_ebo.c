...
Test(ebo_command, basic_test)
{
    ...
    ebo_command(&app, 1);
    cr_assert_str_eq(gui.list_messages->first->data.message, "ebo 1\n");
}

Test(ebo_command, multiple_gui_test)
{
    app_t app;
    gui_t gui;
    node_data_t gui_data;
    ...
    while (node) {
        cr_assert_str_eq(node->data.message, "ebo 1\n");
        node = node->next;
    }
}
